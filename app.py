...
if analyze_button:
    ...
    if (st.session_state.selected_source == "code" and not has_code) or (st.session_state.selected_source == "files" and not has_files) or (st.session_state.selected_source == "github" and not has_github):
        ...
        with st.spinner("🤖 AI is analyzing your code..."):
            try:
                files, data = [], {}
                if st.session_state.selected_source == "files":
                    for file in uploaded_files:
                        files.append(("files", (file.name, file.read(), "text/plain")))
                    active_source = 'files'
                elif st.session_state.selected_source == "code":
                    data["code_content"] = code_input
                    active_source = 'code'
                else:
                ...
                data["review_types"] = json.dumps({"security": review_security, "performance": review_performance, "bugs": review_bugs, "style": review_style, "refactoring": review_refactoring})
                
                response = requests.post(f"{API_URL}/review", files=files if st.session_state.selected_source == "files" else None, data=data) if st.session_state.selected_source == "files" else requests.post(f"{API_URL}/review", data=data)
